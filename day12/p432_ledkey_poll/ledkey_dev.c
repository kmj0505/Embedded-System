...
irqreturn_t sw_isr(int irq, void *unuse)
{
	...
	for(i=0; i<ARRAY_SIZE(key); i++)
	{
		if(irq == sw_irq[i])
		{	
			sw_no = i+1;     //i는 0부터이지만 key는 1부터 시작하므로 +1
			break;
		}
	}
...
int ledkeydev_init(void)
{
	...
	for(i=0; i<ARRAY_SIZE(key); i++)
	{
		result = request_irq(sw_irq[i], sw_isr, IRQF_TRIGGER_RISING, sw_name[i], NULL); //상승엣지일 때 인터럽트 발생후 sw_isr을 호출
		if(result)
		{
