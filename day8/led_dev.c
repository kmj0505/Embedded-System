...
#define leddev_DEV_NAME "leddev"
#define leddev_DEV_MAJOR 240
...
static int leddev_init(void)
{
	...
	result = register_chrdev(leddev_DEV_MAJOR, leddev_DEV_NAME, &leddev_fops);
...
static void leddev_exit(void)
{
	...
	unregister_chrdev(leddev_DEV_MAJOR, leddev_DEV_NAME);
	led_write(0);
	led_free();
}
